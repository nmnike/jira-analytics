...
### Period filter (year / quarter / month)

`QuarterPicker` в шапке `/dashboard` поддерживает выбор месяца внутри квартала. Виджеты должны корректно реагировать:

| Метрика | Месяц выбран | Только квартал |
|---|---|---|
| **Виджет 1** | | |
| Donut статусов | статусы на текущий момент (не зависят от периода) | то же |
| KPI «Всего фактом / Σ план» | за месяц | за квартал |
| KPI «Молчат >14 дней» | от today, не зависит от периода | то же |
| KPI «Закроются в срок» | прогноз всегда к концу квартала | то же |
| Срок проекта (due_date) | абсолютная дата | то же |
| Тренд (часы за 7д) | от today, не зависит | то же |
| Прогноз закрытия (forecast_close_date) | по темпу выбранного периода (месяца или квартала) | по темпу квартала |
| Спарклайн «Активность по неделям» | 8 точек по неделям выбранного периода | 8 точек квартала |
| **Виджет 2** | | |
| План на сотрудника | `RoleCapacityRule × workdays_in_month` если месяц | `× workdays_in_quarter` |
| Факт на сотрудника | ворклоги за месяц | ворклоги за квартал |
| Σ по роли | сумма по сотрудникам периода | то же |
| **Виджет 3** | | |
| Часы по категориям | за месяц | за квартал |

Backend: все 3 endpoint уже принимают `month: Optional[int]` и `quarter_to_dates()` возвращает соответствующий [start, end]. Никаких новых параметров не нужно.
