...
class ResourcePlanningService:
    ...
    def _allocate_hours(
        ...
        День занимается фазой целиком (на величину cap), даже если до закрытия
        фазы осталось меньше. Иначе следующая инициатива подхватывает остаток
        часов того же дня, и два анализа визуально стартуют в один день.
        """
        ...
        while remaining_h > 0.01 and d <= deadline:
            ...
            if cap > 0:
                ...
                # День блокируется на величину cap (фаза «занимает» свой слот
                # целиком). used идёт в hours_allocated как фактически списанные
                # часы; остаток слота не достанется следующей фазе того же дня.
                emp_days[d] -= cap
                remaining_h -= used
                used_total += used
                seg_end = d
