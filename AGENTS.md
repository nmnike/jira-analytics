...
- **SettingsPage** (`/settings`, admin-only): вкладки (порядок — `SettingsPage.tsx`): `connection` (ConnectionCard: Jira credentials), `scope` (ScopeAdmin), `fields` (JiraFieldsCard), `hierarchy` (HierarchyRulesTab), `reasons` (AbsenceReasonsTab), `categories` (**CategoriesTab — кнопка «Пересчитать маппинг по задачам»**), `worktypes`, `calendar` (ProductionCalendarTab + «Синхронизировать» с RU календарём), `ai`, `visibility`, `users` (только admin). Active tab persisted in URL.
